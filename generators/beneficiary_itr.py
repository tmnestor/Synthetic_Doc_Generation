"""Beneficiary individual tax return renderer — ATO NAT 2541 inspired layout.

Renders A4-format individual tax returns with ATO-style header,
identity section with digit boxes for TFN, and Item 13 section
for trust income and franking credits.
"""
...
_LAYOUT_PATH = "config/layouts/beneficiary_itrs.yml"


def _budget(layout: dict, layout_id: str, field: str) -> dict:
    """Look up a field budget for a beneficiary ITR layout."""
...
def _draw_digit_boxes(
    ...
    """Draw individual digit boxes for TFN display."""
...
def render_beneficiary_itr(entry: dict, layout: dict, *, geometry_out: dict | None = None) -> Image.Image:
    """Render a beneficiary individual tax return from ground truth and layout config.
    ...
    Returns:
        PIL Image of the rendered beneficiary ITR.
    """
    fields = entry["fields"]
    ...
    for section in layout.get("sections", []):
        ...
        if sec_type == "header_bar":
            ...
            if title:
                ...
                y += 56
            if subtitle:
                draw.text((margin + 20, y), subtitle, font=font_s, fill="gray")
                y += 32
            ...
            for field_def in section.get("fields", []):
                ...
                label_code = field_def.get("label_code")
                ...
                if fmt == "digit_boxes":
                    ...
                    # Draw item number if present
                    label_x = margin
                    if item_num:
                        font_item = load_font(24, bold=True)
                        draw.text((label_x, y + 2), item_num, font=font_item, fill="#333333")
                        label_x += 50

                    # Draw label code if present
                    if label_code:
                        font_lc = load_font(22, bold=True)
                        draw.text((label_x, y + 2), label_code, font=font_lc, fill="#0066CC")
                        label_x += 36

                    draw.text((label_x, y + 2), label, font=font_b, fill="black")
